...
Use Herdr as usual. You do not open Pairfob first, and you do not mark a session as remote. If `pairfob` is running (it usually starts at login after install), a paired device can attach at any time.
...
Both sides were the same session. Sit down and look at Herdr.

## A brief dropout

The connection reconnects. The credential is still in this browser; you do not scan again. After reconnect the list and the frame follow the computer’s current state.

If `pairfob` restarts on the computer, paired devices remain valid, but an in-flight pairing is void.

## Several devices at once

A tablet or another computer’s browser can pair the same way. What you see and can operate matches the phone. Several attachments still share one herd.
