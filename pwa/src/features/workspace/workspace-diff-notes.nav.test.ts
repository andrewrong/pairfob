...
const { enterWorkspace, leaveWorkspace, loadGitDiff } = await import("./index.ts");
const { sendDiffNotesToAgent } = await import("../operations/controller.ts");
...
async function boot(
  ...
  act(() => {
    ...
    applySnapshot({
      ...
      panes: [{ pane_id: "p1", workspace_id: "w1", agent: options.hasAgent ? "codex" : "", agent_status: "idle" }],
    });
    applyCapabilities({ prompt_agent: options.prompt_agent }, []);
