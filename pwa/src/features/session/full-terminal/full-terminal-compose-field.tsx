...
export function FullTerminalCompose({ send }: FullTerminalComposeProps) {
  ...
  // Subscribe to the compose domain: the initial draft comes from its frozen
  // snapshot, and later changes arrive through the controller feedback below.
  const compose = useCompose();
  const [draft, setDraft] = useState(compose.composeDraft);
