...
const liveSrc = await Bun.file(new URL("./controller.ts", import.meta.url)).text();
const pollingSrc = await Bun.file(new URL("./polling.ts", import.meta.url)).text();
...
describe("refresh contract constants", () => {
  ...
  test("the board screen polls pane.read into cell previews", () => {
    ...
    expect(canRead).toContain('currentScreen() === "board"');
  ...
  test("the production connector forwards finished P2P attempts into the pool channel", () => {
    const connector = liveSrc.slice(
      ...
      liveSrc.indexOf("function monotonicNow()"),
    );
    // The pool receives finished observations only through this callback: it
    // caches pre-bind attempts and routes post-bind ones to the lifecycle
    // listener. Telemetry alone would drop both.
    expect(connector).toContain('track("pwa_p2p", { result, extra })');
    expect(connector).toContain("observeP2PAttempt(observation)");
  });
});
...
describe("pane changes pull agent status forward", () => {
  ...
  test("the pane read path is what triggers it", async () => {
    const read = liveSrc.slice(liveSrc.indexOf("async function performPaneRead"), liveSrc.indexOf("function ensurePaneRefreshBound"));
    const snapshotSrc = await Bun.file(new URL("./snapshot.ts", import.meta.url)).text();
    expect(read).toContain("shouldPullStatus(!same, Date.now(), lastSnapshotAt())");
    expect(snapshotSrc).toContain("noteSnapshotAt(ports.now())");
  });
});
...
describe("poke refresh router", () => {
  ...
  test("Herdr availability transitions refresh configuration before screen data", async () => {
    ...
    const runtimeSrc = await Bun.file(new URL("./runtime.ts", import.meta.url)).text();
    const recovery = runtimeSrc.slice(
      runtimeSrc.indexOf("export async function refreshRuntimeState"),
    );
    expect(recovery.indexOf("await refreshHerdConfig(ports)"))
      .toBeLessThan(recovery.indexOf("await ports.refreshFromSession()"));
  });
});
