...
| `chat-pi-recovery` | Stable long transcript for fixture lifecycle and production refresh publication. | `await qa.setScene("chat-pi-recovery")`; `qa.clearCalls()`; `qa.setConnected(false)`; `qa.emit({type:"reconnecting",message:"QA reconnect"})`; `qa.appendChatTurn()`; `qa.setConnected(true)`; `qa.emit({type:"connected"})`; `qa.emit({type:"poke",reason:"agent_update"})`; `await qa.refreshChat()`. Confirm one `agentTrace` read, appended content, and no mutation replay. For foreground exercise, background/foreground the tab and record visibility plus calls. This is fixture lifecycle coverage, not a real socket reconnect. |
...
Tool details use scene-backed fixture data through the existing `agentTraceDetail` API. `appendChatTurn()` only appends a bounded deterministic backend turn; `refreshChat()` uses the production `refreshAgentTrace` controller. The connection/event actions do not emulate a network handshake or Pi session files, and these fixtures do not duplicate the chat UI.
